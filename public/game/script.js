...
$(function() {
    ...
    // Bot loop to randomly switch tracks every few seconds to keep the game alive
    setInterval(() => {
        const nodes = $('.node').not('.extension, .root, .leaf, .extended_leaf');
        if (nodes.length > 0) {
            const randomNode = nodes.eq(Math.floor(Math.random() * nodes.length));
            randomNode.click();
        }
    }, 2500);
});