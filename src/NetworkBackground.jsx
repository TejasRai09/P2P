  ...
  useEffect(() => {
    ...
    const connectionShader = {
        ...
        void main() {
            ...
            finalPos += perpendicular * snoise(vec3(pathIndex * 0.08, t * 0.6, uTime * 0.15)) * 0.12;
        ...
        function generateCrystallineSphere() {
            ...
            for (let layer = 1; layer <= layers; layer++) {
                ...
                for (let i = 0; i < numPoints; i++) {
                    ...
                    const node = new Node(pos, layer, (layer === layers || Math.random() < 0.3) ? 1 : 0);
                    ...
                    if (layer > 1) {
                        ...
                        for (let j = 0; j < Math.min(3, prevLayerNodes.length); j++) {
                            node.addConnection(prevLayerNodes[j], Math.max(0.3, 1.0 - (pos.distanceTo(prevLayerNodes[j].position) / (radius * 2))));
                        }
                ...
                for (let i = 0; i < layerNodes.length; i++) {
                    ...
                    for (const nearNode of nearby) {
                        if (node.position.distanceTo(nearNode.position) < radius * 0.8 && !node.isConnectedTo(nearNode)) node.addConnection(nearNode, 0.6);
                    }
                }
            }
        }
    ...
    const onClick = (e) => { if (!config.paused) triggerPulse(e.clientX, e.clientY); };
    ...
    function animate() {
        ...
        if (!config.paused && nodesMesh && connectionsMesh) {
    ...
    createNetworkVisualization(config.currentFormation, 0.6); // Reduced density for performance
    animate();
